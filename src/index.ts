...
import Imap, { ImapMessageBodyInfo } from 'node-imap';
...
const config = {
  auth: {
    user: process.env.EMAIL_USER || "demo.user@demo.com",
    pass: process.env.EMAIL_PASS || "Welcome2803!"
  },
  smtp: {
    host: process.env.SMTP_HOST || "mail.demo.com",
    port: parseInt(process.env.SMTP_PORT || "25"),
    tls: process.env.SMTP_TLS === 'true'
  },
  imap: {
    host: process.env.IMAP_HOST || "mail.demo.com",
...
class EmailServer {
  ...
  private async handleSendEmail(args: any): Promise<ToolResult> {
    if (!args.to || !args.subject || !args.text) {
      throw new McpError(
        ErrorCode.InvalidParams,
        'Missing required parameters: to, subject, text'
      );
    }
  ...
  private async handleReceiveEmail(args: any): Promise<ToolResult> {
    ...
    return new Promise((resolve, reject) => {
      this.imap.once('ready', () => {
        this.imap.openBox('INBOX', false, async (err: Error | null, box: any) => {
          if (err) {
            reject(new McpError(ErrorCode.InternalError, `Failed to open inbox: ${err.message}`));
            return;
          }

          const messagePromises: Promise<EmailMessage>[] = [];
          ...
          fetch.on('message', (msg) => {
            const messagePromise = new Promise<EmailMessage>((resolveMsg, rejectMsg) => {
              const chunks: Buffer[] = [];
              ...
              msg.on('body', (stream: NodeJS.ReadableStream, info: ImapMessageBodyInfo) => {
                ...
                stream.once('end', async () => {
                  try {
                    ...
                    const to = Array.isArray(parsedEmail.to) ? parsedEmail.to.map((address: AddressObject) => address.text).join(', ') : parsedEmail.to?.text || '';
                    const cc = Array.isArray(parsedEmail.cc) ? parsedEmail.cc.map((address: AddressObject) => address.text).join(', ') : parsedEmail.cc?.text || '';
                    const bcc = Array.isArray(parsedEmail.bcc) ? parsedEmail.bcc.map((address: AddressObject) => address.text).join(', ') : parsedEmail.bcc?.text || '';
